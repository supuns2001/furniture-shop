...
export function EditProductForm({
  ...
  const [isEmiEligible, setIsEmiEligible] = useState(product.isEmiEligible);

  // Images state (max 3, hydrated from product.images)
  ...
  // Variants state (hydrated from product.variants)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        id: product.id, // required for PUT
        name,
        description,
    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-6xl mx-auto pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-heading text-foreground">Edit Product</h1>
            <p className="text-sm text-muted-foreground mt-1">Modify registered details for {product.name}.</p>
          </div>
        </div>
